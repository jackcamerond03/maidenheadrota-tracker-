...
def build_timeline_html(df, sort_by="Chronological"):
    """Build a simple HTML/CSS timeline (no plotly).
    
    Args:
        df: shifts dataframe
        sort_by: "Chronological" (by earliest shift start) or "Alphabetical" (by name)
    """
    if df.empty:
        return ""
    ...
    # Sort employees by chosen method
    if sort_by == "Chronological":
        emp_start_times = {}
        for emp in df["Employee"].unique():
            emp_df = df[df["Employee"] == emp]
            earliest = min(to_minutes(s) for s in emp_df["Start"])
            emp_start_times[emp] = earliest
        employees = sorted(emp_start_times.keys(), key=lambda e: emp_start_times[e])
    else:  # Alphabetical
...
st.sidebar.write("---")
st.sidebar.header("⏱️ Timeline display")
timeline_sort = st.sidebar.radio("Sort by", ["Chronological", "Alphabetical"], horizontal=False)
...
st.warning(
    "⚠️ **Data retention**: This rota runs on Streamlit Cloud's free tier. "
    "If the app is unused for 24–48 hours, the database will reset and all shifts will be cleared. "
    "For permanent data storage, upgrade to a paid plan or contact your administrator."
)
...
if view == "Day":
    ...
    if day_df.empty:
        ...
        timeline_html = build_timeline_html(day_df, sort_by=timeline_sort)
        ...
        st.markdown("### ✏️ Edit shifts")
        st.caption("Click on any Start or End time to change it, or use the delete button.")
        
        edit_df = day_df[["Employee", "Role", "Start", "End"]].copy()
        ...
        edited = st.data_editor(
            ...
            column_config={
                ...
                "Start": st.column_config.SelectboxColumn("Start", options=TIME_CHOICES),
                "End": st.column_config.SelectboxColumn("End", options=TIME_CHOICES),
        ...
        if not edited.equals(edit_df):
            for i, (_, orig_row) in enumerate(day_df.iterrows()):
                ...
                elif orig_row["Start"] != edit_row["Start"] or orig_row["End"] != edit_row["End"]:
                    if to_minutes(edit_row["End"]) <= to_minutes(edit_row["Start"]):
                        st.error(f"Row {i+1}: End time must be after the start time.")
                    elif has_conflict(all_shifts, orig_row["Employee"], active_date, 
                                    edit_row["Start"], edit_row["End"], exclude_id=sid):
                        st.error(f"Row {i+1}: That would overlap another shift for {orig_row['Employee']}.")
                    else:
                        update_shift(sid, edit_row["Start"], edit_row["End"])
            st.rerun()
...
# --------------------------------------------------------------------------- #
    ...
    if week_df.empty:
        ...
        timeline_html = build_timeline_html(week_df, sort_by=timeline_sort)
        st.markdown(timeline_html, unsafe_allow_html=True)
